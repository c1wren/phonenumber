    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Start(ref e) => match e.name() {
                QName(b"phoneNumberMetadata") => continue,

                QName(b"territories") => result.extend(territories(reader)?),

                name => ignore(reader, name)?,
            },

            Event::End(ref e) if e.name() != QName(b"phoneNumberMetadata") => {
            ...
            Event::End(ref e) if e.name() == QName(b"phoneNumberMetadata") => return Ok(result),
...
fn territories<R: BufRead>(reader: &mut Reader<R>) -> Result<Vec<Metadata>, error::LoadMetadata> {
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Start(ref e) => match e.name() {
                QName(b"territory") => result.push(territory(reader, e)?),

                name => ignore(reader, name)?,
            },

            Event::End(ref e) if e.name() == QName(b"territories") => return Ok(result),
...
fn territory<'a, R: BufRead>(
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Start(ref e) => match e.name() {
                name @ QName(b"references") | name @ QName(b"areaCodeOptional") => {
                    ignore(reader, name)?
                }

                name @ QName(b"generalDesc") => {
                    meta.general = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"fixedLine") => {
                    meta.fixed_line = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"mobile") => meta.mobile = Some(descriptor(reader, &meta, name)?),

                name @ QName(b"tollFree") => {
                    meta.toll_free = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"premiumRate") => {
                    meta.premium_rate = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"sharedCost") => {
                    meta.shared_cost = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"personalNumber") => {
                    meta.personal_number = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"voip") => meta.voip = Some(descriptor(reader, &meta, name)?),

                name @ QName(b"pager") => meta.pager = Some(descriptor(reader, &meta, name)?),

                name @ QName(b"uan") => meta.uan = Some(descriptor(reader, &meta, name)?),

                name @ QName(b"emergency") => {
                    meta.emergency = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"voicemail") => {
                    meta.voicemail = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"noInternationalDialling") => {
                    meta.no_international = Some(descriptor(reader, &meta, name)?)
                }

                name @ QName(b"availableFormats") => {
                ...
                name => {
                    return Err(error::Metadata::UnhandledElement {
                        ...
                        name: str::from_utf8(name.into_inner())?.into(),
                    }
                    .into())
                }
            },

            Event::End(ref e) if e.name() == QName(b"territory") => return Ok(meta),
...
fn descriptor<R: BufRead>(
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Start(ref e) => match e.name() {
                name @ QName(b"nationalNumberPattern") => {
                    descriptor.national_number = Some(text(reader, name)?)
                }

                name @ QName(b"exampleNumber") => descriptor.example = Some(text(reader, name)?),
...
fn format<'a, R: BufRead>(
    ...
    loop {
        match reader.read_event_into(&mut buffer)? {
            ...
            Event::Start(ref e) => match e.name() {
                name @ QName(b"leadingDigits") => format.leading_digits.push(text(reader, name)?),
...
fn ignore<R: BufRead>(reader: &mut Reader<R>, name: QName) -> Result<(), error::LoadMetadata> {
    let mut buffer = Vec::new();
...
fn text<R: BufRead>(reader: &mut Reader<R>, name: QName) -> Result<String, error::LoadMetadata> {
    let mut buffer = Vec::new();
